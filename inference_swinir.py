...
#./swinir_valeurs_par_defaut_model_checkpoint.pt

def main():
    parser = argparse.ArgumentParser() # default='lightweight_sr'
    ...
    parser.add_argument('--training_patch_size', type=int, default=200, help='patch size used in training SwinIR. ' #128
                                       'Just used to differentiate two different settings in Table 2 of the paper. '
                                       'Images are NOT tested patch by patch.')
    parser.add_argument('--large_model', action='store_true', help='use large model, only provided for real image sr')
    parser.add_argument('--model_path', type=str,default='superresolution/SwinIR 5m to 2.5m ClearML dataset/models/735000_E.pth') #swinir_valeurs_par_defaut_model_checkpoint.pt'
#                        default='model_zoo/swinir/001_classicalSR_DIV2K_s48w8_SwinIR-M_x2.pth')
    parser.add_argument('--folder_lq', type=str, default="/home/horoma/data/70.SuperResolution/ClearMLDataset/test_predicted5m", help='input low-quality test image folder')
    parser.add_argument('--folder_gt', type=str, default="/home/horoma/data/70.SuperResolution/ClearMLDataset/test/2.5m", help='input ground-truth test image folder')
...
def define_model(args):
    ...
    # use 'pixelshuffledirect' to save parameters
        ...
        model =net(upscale=args.scale, in_chans=3, img_size=(128,128),
                   window_size=8, img_range=1., depths=[6, 6, 6, 6],
                   embed_dim=60, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='pixelshuffledirect', resi_connection='1conv') #96
...
#                    mlp_ratio=2, upsampler='pixelshuffledirect', resi_connection='1conv')
    ...
    # use window_size=7 because JPEG encoding uses 8x8; use img_range=255 because it's sligtly better than 1
        ...
        model = net(upscale=1, in_chans=1, img_size=126, window_size=7,
                    ...
                    mlp_ratio=2, upsampler='', resi_connection='1conv')
        param_key_g = 'params'
    elif args.task == "CARN_V2":
        model = CARN_V2(color_channels=3, mid_channels=64, conv=nn.Conv2d, # mid_channels=64
                single_conv_size=3, single_conv_group=1,
                scale=2, activation=nn.LeakyReLU(0.1),
                SEBlock=True, repeat_blocks=3, atrous=(1, 1, 1)) #repeat_blocks=3
        param_key_g = 'params'
...
def setup(args):
    ...
    if args.task in ['classical_sr', 'lightweight_sr',"CARN_V2"]:
...
def get_image_pair(args, path):
    ...
    if args.task in ['classical_sr', 'lightweight_sr',"CARN_V2"]:
        img_gt = cv2.imread(path, cv2.IMREAD_COLOR| cv2.IMREAD_ANYDEPTH).astype(np.float32) / 255.
        imgname=imgname.strip('_panSharpenedGS')
        img_lq = cv2.imread(f'{args.folder_lq}/{imgname}{imgext}', cv2.IMREAD_COLOR | cv2.IMREAD_ANYDEPTH).astype(
            np.float32) / 255.
